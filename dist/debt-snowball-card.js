var DebtSnowballApp = (() => {
  ...
  var CARD_AUTOPAY_BUDGET_ID = "auto-card-autopay";
  var CARD_AUTOPAY_BUDGET_NAME = "\u{1F4B3} Card Autopay";
  ...
  function syncCardExpenses({ recurringCosts: recurringCosts2 = [], oneTimeCosts = [], spendingBudgets = [], monthKey, todayDay, skips = [] }) {
    ...
    const arrived = dueCard.filter((c) => chargeDayFor(monthKey, c.dueDay) <= todayDay && !skipSet.has(`${monthKey}:${c.id}`));
    ...
    if (fallback && fallback.autoGenerated && fallbackTotal === 0 && !fallback.expenses.some((e) => !e.autoCard)) {
  ...
  function cashExpensesForMonth(spendingBudgets, monthKey) {
    return (spendingBudgets || []).flatMap((b) => (b.expenses || []).filter((e) => !e.autoCard).map((e) => ({ ...e, budgetName: b.name }))).filter((e) => {
  ...
  // src/app/storage.js
  var STORE_URL_PATH = "snowball-store";
  async function ensureStoreDashboard() {
    ...
    try {
      ...
      if (dashboards.some((d) => d.url_path === STORE_URL_PATH)) return;
    } catch (err) {
    }
    try {
  ...
  function deleteExpense(budgetId, expenseId) {
    ...
    if (!deleted) return;
    const skipKey = deleted.autoCard && deleted.costId ? `${appState.workingMonthKey || currentMonthKey()}:${deleted.costId}` : null;
  ...
  function renderPaymentPlan() {
    ...
    const _budgetExpenses = cashExpensesForMonth(
      archiveData ? [] : appState.spendingBudgets,
      _monthKey
    );
    ...
    schedule.forEach((item, index) => {
      ...
      const detailText = item.type === "debt" && item.isSnowballTarget ? "Minimum + Snowball Extra" : item.type === "debt" ? "Minimum Payment" : item.type === "recurring" ? "Paid from bank account" : item.type === "expense" ? "Logged budget spending \u2014 deducted from cash" : item.type === "checkpoint" ? "Resets the running balance for calculations below" : "";
      row.innerHTML = `
  ...
  function renderUI() {
    ...
    const _todayDay = _monthKey === currentMonthKey() ? (/* @__PURE__ */ new Date()).getDate() : 31;
...
One-time bills will be removed, income will be cleared, and interval bills will advance. This cannot be undone.`)) return;
  ...
  // src/app/header.js
  var PANEL_VERSION = "2.4.0";
  var PANEL_BUILD_DATE = "2026-09-22";
