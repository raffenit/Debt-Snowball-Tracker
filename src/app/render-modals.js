...
// ─── Rendering ───────────────────────────────────────────────────────────────
// Ask the user where each newly-uncategorized card charge should live.
// Sequential queue (one modal at a time); 'later' dismissals are suppressed
// for the session — 'autopay'/category/budget choices persist on the cost.
async function _promptCategorizeQueue(costs) {
    if (appState._categorizing) return;
    appState._categorizing = true;
    const dismissed = appState._categorizeDismissed || (appState._categorizeDismissed = new Set());
    try {
        for (const cost of costs) {
            ...
            if (choice === 'later') { dismissed.add(cost.id); continue; }
        ...
        renderUI(); // re-sync routes the charges to their chosen budgets
    }
}
...
function renderUI() {
    ...
    // Every card charge due this month is logged up front — the budgets show
    // committed card spending from day one, each with its real charge date.
    ...
    // Card charges that newly landed in the 💳 Card Autopay catch-all get a
    // one-time categorize prompt — the choice persists on the cost and
    // applies every month.
    if (cardSync.newFallbackCosts?.length) _promptCategorizeQueue(cardSync.newFallbackCosts);

    // Data health: disclose load-time repairs once (flag set by loadBackendData
    // after the raw pre-repair snapshot was preserved).
    ...
    // Sanity checks: advisory anomaly detection on live data. Recomputed every
    // render; the header badge mirrors the count, and the modal auto-opens
    // only when the warning *set* changes — a stable (legit-looking) warning
